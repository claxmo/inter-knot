...
(function () {
    ...
    unsafeWindow.getComments = async (discussionId, cursor = null, isAsc = false) => {
        return (await graphql(
          `query($id: ID!, $cursor: String) { node(id: $id) { ... on Discussion { comments(${isAsc ? 'first' : 'last'}: 20, ${isAsc ? 'after' : 'before'}: $cursor) { pageInfo { startCursor hasPreviousPage, endCursor, hasNextPage } totalCount nodes { author { login avatarUrl } authorAssociation body bodyHTML bodyText createdAt id updatedAt url viewerCanDelete viewerDidAuthor } } } } }`,
    ...
    unsafeWindow.deleteComment = async (commentId) => {
        return (await graphql(
    ...
    unsafeWindow.addComment = async (discussionId, body) => {
        return (await graphql(
