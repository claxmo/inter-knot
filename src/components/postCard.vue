<template>
    <div class="post-card" @click="viewed = true" :class="{mission: isMission, viewed: viewed}">   
        ...
        :src="!isLoaded || isError ? defaultCoverUrl : coverUrl" 
        loading="lazy" 
        @load="loadedHandle" 
        @error="errorHandle"
        ref="cover"
        ...
        <div class="footer">
            ...
            <div class="post-title">
              <img class="icon" v-if="isMission" :src="viewed ? iconMissionViewed : iconMission" />           
              <span class="text">
...
import { defineProps,ref, defineEmits, toRefs, onMounted, watch, computed } from 'vue';
...
const isMission = computed(() => post.value.category.name === '委托');
...
watch(coverVisibility, async (visible) => {
  if (!visible) return
  if (post.value.isNsfw === undefined) {
...
onMounted(async () => {
  ...
  if (img) {
    try {
      coverUrl.value = URL.createObjectURL(await window.getBlob(img.src));
    } catch {
...
.post-card.mission:not(.viewed) .post-title .text {
  background-image: linear-gradient(0, #4661fd, #10bff0);
